...
void usage(void) {
    ...
    printf(" -f <float> Start Radio TX frequency in MHz\n");
    printf(" -e <float> End Radio TX frequency in MHz\n");
    printf(" -S <float> Step Radio TX frequency in MHz\n");
...
int main(int argc, char **argv)
{
    ...
    uint32_t ft_end = DEFAULT_FREQ_HZ;
    uint32_t ft_step = 200000; // 200 kHz
    uint32_t ft_current = DEFAULT_FREQ_HZ;
    ...
    while ((i = getopt_long (argc, argv, "hjif:e:S:s:b:n:z:p:k:r:c:l:t:m:o:ud:", long_options, &option_index)) != -1) {
        switch (i) {
            case 'h':
                usage();
                return -1;
            ...
            case 'f': /* <float> Radio TX frequency in MHz */
                ...
                break;
            case 'e': /* <float> Radio TX frequency in MHz */
                i = sscanf(optarg, "%lf", &arg_d);
                if (i != 1) {
                    printf("ERROR: argument parsing of -f argument. Use -h to print help\n");
                    return EXIT_FAILURE;
                } else {
                    ft_end = (uint32_t)((arg_d*1e6) + 0.5); /* .5 Hz offset to get rounding instead of truncating */
                }
                break;
            case 'S': /* <float> Radio TX frequency in MHz */
                i = sscanf(optarg, "%lf", &arg_d);
                if (i != 1) {
                    printf("ERROR: argument parsing of -f argument. Use -h to print help\n");
                    return EXIT_FAILURE;
                } else {
                    ft_step = (uint32_t)((arg_d*1e6) + 0.5); /* .5 Hz offset to get rounding instead of truncating */
                }
                break;
    ...
    if (strcmp(mod, "CW") == 0) {
        printf("Sending %i CW on %u Hz to %u Hz in %u Hz steps (Freq. offset %d kHz) at %i dBm\n", nb_pkt, ft, ft_end, ft_step, freq_offset, rf_power);
    }
    else if (strcmp(mod, "FSK") == 0) {
        printf("Sending %i FSK packets on %u Hz to %u Hz in %u Hz steps (FDev %u kHz, Bitrate %.2f, %i bytes payload, %i symbols preamble) at %i dBm\n", nb_pkt, ft, ft_end, ft_step, fdev_khz, br_kbps, size, preamble, rf_power);
    } else {
        printf("Sending %i LoRa packets on%u Hz to %u Hz in %u Hz steps (BW %i kHz, SF %i, CR %i, %i bytes payload, %i symbols preamble, %s header, %s polarity) at %i dBm\n", nb_pkt, ft, ft_end, ft_step, bw_khz, sf, 1, size, preamble, (no_header == false) ? "explicit" : "implicit", (invert_pol == false) ? "non-inverted" : "inverted", rf_power);
    }
    ...
    for (cnt_loop = 0; cnt_loop < nb_loop; cnt_loop++) {
        ...
        for (i = 0; i < (int)nb_pkt; i++) {
            ...
            while (ft_current <= ft_end) {
                if (strcmp(mod, "CW") == 0) {
                    pkt.freq_hz = ft_current + (freq_offset * 1000);
                } else {
                    pkt.freq_hz = ft_current;
                }
                ...
                do {
                    ...
                    if (strcmp(mod, "CW") == 0 && curr_time++ > 200) {
                        /* timeout 1s CW */
                        break;
                    }
